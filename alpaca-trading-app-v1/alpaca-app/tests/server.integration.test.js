...
test("changing max open positions in Settings genuinely changes live trading behavior, not just the stored value", async () => {
  const ctx = await startTestServer();
  const base = `http://localhost:${ctx.port}`;

  // Set a tight, easy-to-verify limit via the real Settings save endpoint (the same
  // endpoint the new Settings UI calls) - this is the exact path a user's browser hits.
  let res = await fetch(`${base}/api/settings`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ riskLimits: { maxRiskPerTradePct: 1.0, maxDailyLossPct: 3.0, maxPortfolioExposurePct: 50.0, maxOpenPositions: 2 } }),
  });
  assert.strictEqual(res.status, 200);

  // Confirm it round-trips correctly on read-back
  res = await fetch(`${base}/api/settings`);
  let body = await res.json();
  assert.strictEqual(body.riskLimits.maxOpenPositions, 2, "the new limit should be readable back from settings");
  console.log("PASS: risk limit change round-trips correctly through GET /api/settings");

  // Now prove it actually GOVERNS behavior: open 2 positions (should succeed), the 3rd
  // must be rejected - this is the real test, not just that a number got stored somewhere.
  for (let i = 0; i < 2; i++) {
    res = await fetch(`${base}/api/positions`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ symbol: "AAPL", assetType: "stock", direction: "long", positionSizePct: 1, horizon: "short_term" }),
    });
    assert.strictEqual(res.status, 200, `position ${i + 1} of 2 should succeed under the new limit of 2`);
  }
  res = await fetch(`${base}/api/positions`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ symbol: "AAPL", assetType: "stock", direction: "long", positionSizePct: 1, horizon: "short_term" }),
  });
  assert.notStrictEqual(res.status, 200, "3rd position must be rejected under a max-open-positions limit of 2, proving the setting genuinely governs live behavior");
  const rejectBody = await res.json();
  console.log("PASS: user-changed risk limit (maxOpenPositions=2) genuinely governs live trade rejection -", rejectBody.error);
